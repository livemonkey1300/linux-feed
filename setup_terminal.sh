...
# ==============================================================================
# 🚀 Multi-Distro Terminal & AI Neovim Setup Script
# Works on: Ubuntu, Debian, Zorin OS, Mint, Fedora
# ==============================================================================
...
if [[ "$OS" == "ubuntu" || "$OS" == "debian" || "$OS" == "zorin" || "$OS" == "linuxmint" ]]; then
    ...
    # Upgrade Neovim (v0.11+)
    echo "🚀 Upgrading Neovim to the latest unstable (v0.12+) for modern LSP support..."
    ...
    sudo dnf install -y zsh curl git bat btop ripgrep fzf fd-find ranger xclip neovim
else
    echo "⚠️ Unsupported OS: $OS. Please install dependencies manually."
    exit 1
fi

# 2. Install Lazygit (Cross-distro binary install)
...
# Aliases
# Check if we should use batcat or bat
